package ru.yandex.Sprint.Sprint5.Lesson_4;
...
public class KeywordsGame {
    ...
    public void runGame() {
        ...
        final int newRandom = random.nextInt(KEYWORDS.size());
        int incorrectAnswersCounter = 0;
        while (true) {
            final boolean providedAnswer;
            if (newRandom < RESERVED.size()) {
    ...
    private boolean writeReservedQuestion() {
        ...
        final int randomInt = random.nextInt(RESERVED.size());
        System.out.println(RESERVED.get(randomInt));
        return true;
    }
